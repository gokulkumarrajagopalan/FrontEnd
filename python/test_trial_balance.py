...
def fetch_and_parse(cmp_id=1):
    print(f"Sending Trial Balance request to Tally at {TALLY_URL}...")
    try:
        ...
        # Iterate over all children of ENVELOPE
        for child in root:
        ...
        if parsed_data:
            sync_url = f"{BACKEND_URL}/reports/trailbalance/sync"
            ...
            try:
                ...
                if sync_resp.status_code in [200, 201]:
                    print("✓ Trial Balance successfully synced to database")
                else:
