...
class CurrencySyncer:
    """Syncer for Tally Currencies to Backend."""
    ...
    def sync(self, company_id: int, user_id: int, auth_token: str, device_token: str):
        """Fetches currencies from Tally and syncs them to the backend."""
        # Step 1: Fetch from Tally
        success, currencies = self.client.get_currencies()
        if not success:
            output_result(False, f"Failed to fetch currencies from Tally: {currencies.get('error')}")
        
        # Step 2: Prepare for backend
        for currency in currencies:
            currency['cmpId'] = company_id
            currency['userId'] = user_id
            currency['syncStatus'] = 'SYNCED'
            currency['lastSyncDate'] = datetime.now().isoformat()
        ...
        try:
            ...
            response = requests.post(
                f"{self.backend_url}/currencies/sync",
                json=currencies,
                headers=headers,
                timeout=30
            )
...
def main():
    ...
    syncer = CurrencySyncer(host=args.tally_host, port=args.tally_port, backend_url=args.backend_url)
    success, result = syncer.sync(args.company_id, args.user_id, args.auth_token, args.device_token)
    
    if success:
        output_result(True, f"Successfully synced currencies for company {args.company_id}", data=result)
    else:
