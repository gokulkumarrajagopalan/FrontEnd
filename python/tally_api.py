...
<COLLECTION NAME="ListOpenCompanies"><TYPE>Company</TYPE><FETCH>NAME</FETCH></COLLECTION>
    ...
    def _build_company_request(self) -> str:
        ...
        """
        xml = """<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Collection</TYPE><ID>Collection of Companies</ID></HEADER><BODY><DESC><STATICVARIABLES><SVFROMDATE TYPE="Date">01-Jan-1970</SVFROMDATE><SVTODATE TYPE="Date">01-Jan-1970</SVTODATE><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT></STATICVARIABLES><TDL><TDLMESSAGE><COLLECTION NAME="Collection of Companies" ISMODIFY="No"><TYPE>Company</TYPE><FETCH>GUID,ALTERID,MASTERID,GUID,NAME,STATE,STARTINGFROM,BOOKSFROM,ENDINGAT,LASTVOUCHERDATE,CMPVCHID,ALTVCHID,ALTMSTID,ISAGGREGATE,BASICCOMPANYFORMALNAME,COMPANYCHEQUENAME,COMPANYCONTACTPERSON,COMPANYCONTACTNUMBER,EMAIL,WEBSITE,PHONENUMBER,CMPMOBNO,MOBILENO,_ADDRESS1,_ADDRESS2,_ADDRESS3,_ADDRESS4,_ADDRESS5,STATENAME,PINCODE,COUNTRYNAME,VATTINNUMBER,CORPORATEIDENTITYNO,UDF:CORPORATEIDENTITYNO,INTERSTATESTNUMBER,TANUMBER,STREGNUMBER,CMPPFCODE,INCOMETAXNUMBER,GSTREGISTRATIONTYPE,GSTAPPLICABLEDATE,ISGSTCESSON,GSTDETAILS.LIST,HSNDETAILS.LIST,DESTINATION</FETCH><FILTERS>GroupFilter</FILTERS></COLLECTION><SYSTEM TYPE="FORMULAE" NAME="GroupFilter">$isaggregate = "No"</SYSTEM></TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>"""
        return xml
    ...
    def _parse_company_element(self, company: dict) -> Optional[dict]:
        ...
        try:
            ...
            company_info = {
                ...
                "lastVoucherDate": get_text(company.get("LASTVOUCHERDATE", "")),
